...
Data simulation 
```{r, eval=FALSE, echo=FALSE}
simulate_model_1.2 <- function(n_per_type = 1000, noise_sd = 0.2, seed = 45) {
  # Define sampler types and their true intercepts
  sampler_types <- c("LCS", "ISCO", "GB", "GBH")
...
simulate_model_1.3 <- function(n_per_type = 100, noise_sd = 0.2, seed = 45) {
  ...
  block_effects <- c(Block1 = 0, Block2 = 0.2) # Expressed in SDs of the analyte; block 1 will unaffect results
  ...
  sim_data$C_obs <- mapply(function(analyte, sampler, block) {
    ...
    sd_conc <- base_sd[analyte]
    sampler_effect <- true_intercepts[sampler]
    block_effect <- block_effects[block] #* sd_conc # Convert block effect to scale of the analyte
  ...
  sim_data <- sim_data %>% select(analyte_abbr, S, block, C_obs)
  
  return(sim_data)
}

# Generate simulated data for Model 1.3
...
data1.3 <- list(
  C_obs = sim_data_1.3$C_obs,                # Observed concentrations (not standardized)
...
m1.3 <- ulam(
  alist(
    ...
    # Priors for block-specific intercepts
    gamma[B] ~ dnorm(g_bar, sigma_g),
    
    # Hyper-priors
    ## for analyte-specific intercepts
    a_bar ~ dnorm(0, 0.5),
    sigma_a ~ dexp(1),
    ## for block-specific intercepts
    g_bar ~ dnorm(0, 0.5),
    sigma_g ~ dexp(1),
...
# Link function for posterior predictions
link_function <- function(post, data) {
  n_samples <- nrow(post$beta[, , 1])  # Number of posterior samples
  n_analytes <- data$K_A               # Number of analytes
  n_samplers <- data$K_S               # Number of sampler types
  ...
  for (a in 1:n_analytes) {
    for (s in 1:n_samplers) {
      for (b in 1:n_blocks) {
        predictions[, a, s, b] <- post$alpha[, a] + post$beta[, a, s] + post$gamma[, b]
      }
    }
  }
...
# Summarize predictions into a dataframe
prediction_summary <- data.frame()

for (a in 1:data1.3$K_A) {
  for (s in 1:data1.3$K_S) {
    for (b in 1:data1.3$K_B) {
      samples <- posterior_predictions[, a, s, b]
      summary_row <- data.frame(
        analyte_abbr = a,
        sampler_type = levels(sim_data_1.3$S)[s],
        block = levels(sim_data_1.3$block)[b],
        mean_prediction = mean(samples)
      )
      prediction_summary <- rbind(prediction_summary, summary_row)
    }
...
prediction_summary %>%
  group_by(analyte_abbr, block) %>%
  spread(key = sampler_type, value = mean_prediction)

```

Results: 

### Model 1.4 - Multilevel model with sampler, analyte, and block effects with MVN distribution between analytes
Data simulation
```{r, eval=FALSE, echo=FALSE}
```
