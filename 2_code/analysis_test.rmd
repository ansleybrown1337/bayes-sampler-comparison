...
Model fit
```{r, eval=FALSE}
# Prepare data for the centered model - use data1.3 for now since it should be the same results
data1.4 <- list(
  C_obs = sim_data_1.3$C_obs_standardized,  # Standardized observed concentrations
  S = as.numeric(as.factor(sim_data_1.3$S)), # Sampler type index (1-4)
  A = sim_data_1.3$analyte_abbr,             # Analyte index (1-9)
...
m1.4_noncentered <- ulam(
  alist(
    ...
    # Analyte-specific intercepts (non-centered parameterization)
    ...
    # Analyte-specific sampler effects (non-centered parameterization)
    ...
    # Analyte-specific block effects (non-centered parameterization)
  ...
  warmup = 2000             # Number of warmup iterations (optional, default is iter/2)
)
...
Summary of results
```{r, eval=FALSE}
precis(m1.4_noncentered, depth = 2)

```

```{r}
traceplot(m1.4_noncentered)
```

Posterior predictions
```{r, eval=FALSE}
# Extract posterior samples
post1.4 <- extract.samples(m1.4_noncentered)

# Updated link function for posterior predictions
link_function_1.4 <- function(post1.4, data) {
  n_samples <- nrow(post1.4$alpha)       # Number of posterior samples
  n_analytes <- data$K_A                # Number of analytes
  n_samplers <- data$K_S                # Number of sampler types
  n_blocks <- data$K_B                  # Number of blocks
  ...
  for (a in 1:n_analytes) {
    for (s in 1:n_samplers) {
      for (b in 1:n_blocks) {
        ...
        predictions[, a, s, b] <- post1.4$mu_alpha +
                                  post1.4$z_alpha[, a] * post1.4$sigma_alpha +
                                  post1.4$beta[, a, s] +
                                  post1.4$gamma[, a, b]
      }
    }
...
# Generate predictions
posterior_predictions_1.4 <- link_function_1.4(post1.4, data1.4)

# Initialize array for back-transformed predictions
back_transformed_predictions_1.4 <- array(NA, dim = dim(posterior_predictions_1.4))
...
for (a in 1:data1.4$K_A) {
  ...
  # Back-transform predictions for the analyte
  back_transformed_predictions_1.4[, a, , ] <- posterior_predictions_1.4[, a, , ] * analyte_sd + analyte_mean
}

# Summarize posterior predictions
# Initialize a summary dataframe
prediction_summary_1.4 <- data.frame()
...
for (a in 1:data1.4$K_A) {
  for (s in 1:data1.4$K_S) {
    for (b in 1:data1.4$K_B) {
      # Extract samples for the current analyte, sampler, and block
      samples <- back_transformed_predictions_1.4[, a, s, b]
      ...
      prediction_summary_1.4 <- rbind(prediction_summary_1.4, summary_row)
    }
  }
}

# Refine the summary table: Rows as analytes, columns as sampler types
refined_summary_table_1.4 <- prediction_summary_1.4 %>%
  select(analyte_abbr, sampler_type, mean_prediction, block) %>%
  pivot_wider(names_from = sampler_type, values_from = mean_prediction)

# View the refined summary table
refined_summary_table_1.4
...
```{r}
final_model <- m1.4_noncentered
final_post <- post1.4 #put posterior predictions here
final_post_back_transformed <- back_transformed_predictions_1.4 #put back-transformed posterior predictions here
final_data <- sim_data_1.4
```
