...
png('../figs/sampler_effects_sim.png', width = 800, height = 600, res = 300)
...
# Example usage
png('../figs/sampler_contrasts_sim.png', width = 800, height = 600, res = 300)
...
```{r}
png('../figs/predictions_with_observed_sim.png', width = 800, height = 600, res = 300)
...
# Recalibrate the m1.4_nc model using the real data
m1.4_nc_real <- update(m1.4_nc, data = dat)
```

Summary
```{r}
# Check summary of recalibrated model
precis(m1.4_nc_real, depth = 2)
```

Generate posterior predictions and back-transformed predictions
```{r}
# Extract posterior samples
post1.4_nc_real <- extract.samples(m1.4_nc_real)

# Updated link function for posterior predictions
link_function_1.4_nc <- function(post, data) {
  n_samples <- nrow(post$beta[, , 1])  # Number of posterior samples
  n_analytes <- data$K_A               # Number of analytes
  n_samplers <- data$K_S               # Number of sampler types
  n_blocks <- data$K_B                 # Number of blocks
  ...
  for (a in 1:n_analytes) {
    for (s in 1:n_samplers) {
      for (b in 1:n_blocks) {
        # Compute predictions using the existing posterior parameters
...
# Generate predictions
posterior_predictions_real <- link_function_1.4_nc(post1.4_nc_real, dat)

# Initialize array for back-transformed predictions
back_transformed_predictions_1.4_nc_real <- array(NA, dim = dim(posterior_predictions_real))

# Back-transform predictions for each analyte
for (a in 1:dat$K_A) {
  analyte_mean <- mean(d$result[d$A == a], na.rm = T)  # Replace with actual analyte means
  analyte_sd <- sd(d$result[d$A == a], na.rm = T)      # Replace with actual analyte SDs
  
  # Back-transform predictions for the analyte
  back_transformed_predictions_1.4_nc_real[, a, , ] <- posterior_predictions_real[, a, , ] * analyte_sd + analyte_mean
}
...
for (a in 1:dat$K_A) {
  for (s in 1:dat$K_S) {
    for (b in 1:dat$K_B) {
      samples <- back_transformed_predictions_1.4_nc_real[, a, s, b]
      
      # Skip if all samples are NA or NaN
      if (all(is.na(samples))) {
        cat("Found NA values in predictions for:\n")
        cat("  Analyte:", a, "\n")
        cat("  Sampler:", s, "\n")
        cat("  Block:", b, "\n")
        print(head(samples))  # Print a preview of the problematic samples
        next
      }
      
      # Add summary statistics, ignoring NA values
      summary_row <- data.frame(
        analyte_abbr = a,
...
Select final model and 'observed data' for comparison
```{r}
final_model <- m1.4_nc_real
final_post <- post1.4_nc_real #put posterior predictions here
final_post_back_transformed <- back_transformed_predictions_1.4_nc_real #put back-transformed posterior predictions here
final_data <- d
```
...
png('../figs/sampler_effects.png', width = 800, height = 600, res = 300)
plot_sampler_effects(
  posterior_samples = final_post, 
...
png('../figs/sampler_contrasts.png', width = 800, height = 600, res = 300)
plot_sampler_contrasts(
  posterior_samples = final_post, 
  sampler_mapping = sampler_mapping
)
...
```{r}
# Example usage
png('../figs/predictions_with_observed.png', width = 800, height = 600, res = 300)
plot_predictions_with_observed(
  back_transformed_predictions = final_post_back_transformed, 
  data = d, 
  sampler_mapping = sampler_mapping, 
