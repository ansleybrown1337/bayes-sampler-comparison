...
# Prepare data for the model
data1.6 <- list(
  C_obs = array(as.vector(sim_data_1.6$C_obs_standardized)),     # RESPONSE: array!
  S = as.integer(as.factor(sim_data_1.6$S)),             # index
  A = as.integer(sim_data_1.6$analyte_abbr),             # index
  B = as.integer(as.factor(sim_data_1.6$block)),         # index
  TRT = as.integer(as.factor(sim_data_1.6$treatment)),   # index
  N = nrow(sim_data_1.6),
...
```{r}
plot_treatment_effects <- function(posterior_samples, treatment_mapping) {
  if (!"delta" %in% names(posterior_samples)) {
    stop("Treatment effects 'delta' not found in posterior samples.")
  }

  n_treatments <- dim(posterior_samples$delta)[3]  # Number of treatments
  n_analytes <- dim(posterior_samples$delta)[2]    # Number of analytes
  n_samples <- dim(posterior_samples$delta)[1]     # Number of posterior samples

  # Average treatment effects across analytes and samples
  treatment_effects <- matrix(NA, nrow = n_samples, ncol = n_treatments)
  for (t in 1:n_treatments) {
    treatment_effects[, t] <- rowMeans(posterior_samples$delta[, , t], na.rm = TRUE)
  }
  ...
  y_max <- max(apply(treatment_effects, 2, function(col) max(density(col)$y, na.rm = TRUE)))
  ylim <- c(0, y_max * 1.1)

  # Plot posterior densities for treatment effects
  colors <- c("#8080FF", "#F98400", "#00A08A", "#E2AD00", "#FF8080", "#80FF80")
  plot(NULL, xlim = xlim, ylim = ylim, 
       xlab = "Treatment Effect (Averaged over Analytes)", ylab = "Density", 
       main = "Posterior Treatment Effects")

  for (t in 1:n_treatments) {
    dens(treatment_effects[, t], col = colors[t], lwd = 3, add = TRUE)
  }

  # Add standard normal prior line
  curve(dnorm(x, mean = 0, sd = 1), from = xlim[1], to = xlim[2], lwd = 2, lty = 2, col = "black", add = TRUE)

  # Add legend
  legend("topright", legend = c(names(treatment_mapping), "Prior"), 
         col = c(colors[1:n_treatments], "black"), lwd = 3, lty = c(rep(1, n_treatments), 2))
}

# Simulated data posterior
png('../figs/treatment_effects_sim.png', width = 2000, height = 1200, res = 300)
plot_treatment_effects(final_post, trt_mapping)
dev.off()
...
plot_sampler_effects <- function(posterior_samples, sampler_mapping) {
  ...
  plot(NULL, xlim = xlim, ylim = ylim, 
       ...
       # main = "Posterior Sampler Effects (No ISCO Duplicates)")
...
#png('../figs/sampler_effects_noDupe.png', width = 2000, height = 1200, res = 300)
...
plot_sampler_contrasts <- function(posterior_samples, sampler_mapping) {
  ...
  plot(NULL, xlim = xlim, ylim = ylim, 
       xlab = "Contrast Effect (Averaged over Analytes, Blocks, and Treatments)", ylab = "Density", 
       main = "Posterior Sampler Contrasts")
       #main = "Posterior Sampler Contrasts (No ISCO Duplicates)")
...
#png('../figs/sampler_contrasts_noDupe.png', width = 3000, height = 1200, res = 300)
...
plot_treatment_effects(posterior_samples_real, trt_mapping)
dev.off()
...
plot_posterior_facets <- function(
  posterior_array, sampler_mapping, analyte_mapping, observed_means,
  colors = c("#8080FF", "#F98400", "#00A08A", "#E2AD00", "#800080", "#008000", "#5BBCD6", "#CC79A7", "#AAAAAA"),
  ylab = "Density", n_cols = 3
) {
  ...
  for (a in 1:n_analyte) {
    dens_list <- lapply(1:n_sampler, function(s) {
      vals <- if (length(dim(posterior_array)) > 3) {
        apply(posterior_array[, a, s, , , drop = FALSE], 1, mean, na.rm = TRUE)
      } else {
        posterior_array[, a, s]
      }
    ...
    plot(
      dens_list[[1]], type = "l", lwd = 3, col = colors[1], xlim = xlim, ylim = ylim,
      xlab = "", ylab = ylab, main = analyte_names[a], cex.main = 1.1
    )
    if (n_sampler > 1) {
      for (s in 2:n_sampler) {
        lines(dens_list[[s]], lwd = 3, col = colors[s])
      }
    }
    for (s in 1:n_sampler) {
      obs_val <- observed_means %>%
        filter(analyte_type == analyte_names[a], sampler_type == sampler_labels[s]) %>%
        pull(observed_mean)
      if (length(obs_val) == 1 && !is.na(obs_val)) {
        abline(v = obs_val, col = colors[s], lwd = 2, lty = 2)
      }
    }
  ...
  legend(
    ...
    legend = sampler_labels, col = colors[1:n_sampler], lwd = 3,
    horiz = TRUE, bty = "n", cex = 1.3, xjust = 0.5
  )
  par(xpd = FALSE)
}
...
# or use png(...) for higher clarity if preferred
...
plot_posterior_facets(
  ...
  analyte_mapping = analyte_mapping,
  observed_means = observed_means,
  n_cols = 3,   # or adjust as you wish
  colors = c("#8080FF", "#F98400", "#00A08A", "#E2AD00") # add more if >4 samplers
)
dev.off()
