...
m2.4 <- ulam(
    ...
    ), data=dlist, chains=4, cores=4, threads=2) #test running this with threads=2 to speed it up
```

```{r}
# model 2.5 - see https://speakerdeck.com/rmcelreath/statistical-rethinking-2022-lecture-17?slide=36
# some notes for this model (in development)
# stratifying by treatment, sampler, and analyte;
  # should analyte have it's own slope and intercept? (i.e., bA[A] and g[A]) # Yes - to capture the different baseline and and linear relationships for each analyte
# using partial pooling around block only
# treating C_true as unobserved data
# non-centered model
# TODO: UNTESTED 10 Dec 2024
...
m2.5 <- ulam(
    alist(
        ...
        sigma_a ~ dexp(1),
        sigma_j ~ dexp(1),
        gq> vector[B]:a <<- a_bar + b*sigma_j
        
    ) , data=dlist , chains=4 , cores=4, threads=2)


# Notes for full causal model in Ch2:

# irrigation is not a cluster (i.e., b/c order matters unlike block)
# Analytes may end up being post stratified; we may need to use a different model for each analyte?
